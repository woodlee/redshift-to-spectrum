...
1. This script unloads data in a newline-delimited text format, and therefore is only suitable for infrequently-queried data. For data you expect to work with more often you should consider some other alternative that will allow you to unload to a column-delimited format such as Parquet. [Spectrify](https://github.com/hellonarrativ/spectrify) might be a good choice, but do be aware that as of this writing it has [issues with round-trip handling of null values](https://github.com/hellonarrativ/spectrify/issues/6).
1. The script uses the `get_cluster_credentials` call on the Boto3 Redshift client to get cluster login creds, so whatever IAM context you run this under will need to have that permission (and several others). Changing the script to accept credentials from the environment or command line should be easy though.
...
|---|---|---|
|--region|AWS region for the Redshift cluster and S3 bucket|`us-east-1`|
|--s3-bucket|S3 bucket in which unloads will be stored|`my-unloads-bucket`|
|--s3-path-prefix|Optional S3 path prefix for specifying where unloads are stored|`redshift-unloads`|
|--cluster|Name of the Redshift cluster being unloaded from|`my-cluster`|
|--db|Name of the database within the cluster that contains the table to be unloaded|`my-db`|
|--schema|Name of the schema that contains the table to be unloaded|Change depending on your needs|
|--table|Name of the table to be unloaded|Change depending on your needs|
|--db-user|Redshift user name to log in as when running UNLOADs and other queries. Will need to be pre-provisioned by you.|`awsglue`|
|--iam-role-arn|ARN of the AWS IAM role used in the Redshift UNLOAD command|arn:aws:iam::111111111111:role/My-Redshift-Role|
|--audit-col|Optional name of the column used to select a subset of the tables records for unloading (e.g., a last_updated timestamp or similar). Pick something stable.|`timestamp`|
...
|--audit-max-val|Optional, exclusive maximum value of the `audit_col` used to select records for unloading. If absent, no maximum bound is applied. This will be used exactly as provided in the WHERE clause of SQL queries, so the source DB will be responsible for interpreting e.g. its time zone.|Change depending on your needs|
